...
let
  ...
  skelPackage =
    pkgs.runCommand "konductor-skel"
      {
  ...
  vscodeDefaultSettings = pkgs.writeText "vscode-settings.json" (
    builtins.toJSON {
      # Window
  ...
  mkKonductorService =
    {
      ...
      afterServices ? [
        ...
        "konductor-pki-permissions.service"
      ],
      documentation ? [ ],
    ...
    }:
    {
      "konductor-${serviceName}@" = {
        description = "Konductor ${lib.toUpper (builtins.substring 0 1 serviceName)}${
          builtins.substring 1 (builtins.stringLength serviceName) serviceName
        } for %i";
        ...
        etc = {
          ...
          "konductor/input-sources.env".text =
            ...
            lib.concatStringsSep "\n" (
              lib.mapAttrsToList (name: input: "${name}=${input.outPath or input}") inputsWithPath
            )
            + "\n";
        };
            ...
            serviceConfig = {
              ...
              ExecStart = pkgs.writeShellScript "konductor-verify" ''
                ...
                if [ -f /etc/konductor/pki/vm/ca.crt ]; then
                  ...
                  }/bin/python3 -m pki status 2>/dev/null || echo "  · pki status: failed to run"
                else
            ...
            let
              orchestratorScript = pkgs.writeShellScript "konductor-init-start" ''
                ...
                EOF
                                    ...
                                    restty)
                                      cat >> "$DROPIN_PATH/50-config.conf" << EOF
                ExecStart=/bin/sh -c '. $CERT_ENV_FILE && exec ${
                  (import ../programs/restty-web { inherit pkgs lib; }).server
                }/bin/restty-web-server --port \''${PORT} --cert \$CERT_PATH --key \$KEY_PATH --writable --working-directory /workspace'
                EOF
                                      ;;
                                    ghostty)
                                      cat >> "$DROPIN_PATH/50-config.conf" << EOF
                ExecStart=/bin/sh -c '. $CERT_ENV_FILE && exec ${
                  (import ../programs/ghostty-web { inherit pkgs lib; }).server
                }/bin/ghostty-web-server --port \''${PORT} --cert \$CERT_PATH --key \$KEY_PATH --writable --working-directory /workspace'
                EOF
                                      ;;
                                    vscode)
                                      cat >> "$DROPIN_PATH/50-config.conf" << EOF
